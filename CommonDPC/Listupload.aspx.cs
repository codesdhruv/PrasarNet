    ...
    public partial class Listupload : System.Web.UI.Page
    {
        ...
        protected void lbdesc_Click(object sender, EventArgs e)
        {
            ...
            lbluploadexceltext.Text = "DPC Description ::: " + Session["DPCdesc"].ToString();
                //+ " - promotion to the post : " + Session["DPCpostname"].ToString() ;
        ...
        protected void btnaddaDPC_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                lbldpcdsr.Text = "Upload Excel for DPC: " + txtdpcdescr.Text + " - promotion to the post : " + txtPostName.Text;
        ...
        protected void grdDPCdetails_OnRowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
        {
            ...
            //}
            if (e.Row.RowType == DataControlRowType.DataRow && e.Row.RowIndex != grdDPCdetails.EditIndex)
            {
                (e.Row.FindControl("lbDelete") as LinkButton).Attributes["onclick"] = "return confirm('Do you want to delete this row?');";
            }
        ...
        protected void lbDelete_Click(object sender, EventArgs e)
        {
            LinkButton lb = (LinkButton)sender;
            int dpcid = Convert.ToInt32(lb.CommandArgument);
            //lb.Attributes["onclick"] = "return confirm('Do you want to delete this row?');";
            DPCDetailsDelete(dpcid);

        }

        protected void DPCDetailsDelete(int dpcid)
        {
            int customerId;
            string constr = ConfigurationManager.ConnectionStrings["PRASARNETConnectionString"].ConnectionString;
            ...
            ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Details Deleted');", true);
            BindGridviewDPCList();

        }
        protected void grdDPCdetails_RowEditing(object sender, GridViewEditEventArgs e)
        {
            grdDPCdetails.EditIndex = e.NewEditIndex;
            BindGridviewDPCList();
        }
        protected void grdDPCdetails_OnRowCancelingEdit(object sender, EventArgs e)
        {
            grdDPCdetails.EditIndex = -1;
            BindGridviewDPCList();
        }
        protected void DPCDetailsUpdate(int dpcid, string description, string postname)
        {
            int customerId;
            string constr = ConfigurationManager.ConnectionStrings["PRASARNETConnectionString"].ConnectionString;
            ...
            ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Details Updated');", true);
            BindGridviewDPCList();

        }
        protected void grdDPCdetails_OnRowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            GridViewRow row = grdDPCdetails.Rows[e.RowIndex];
            int dpcid = Convert.ToInt32(grdDPCdetails.DataKeys[e.RowIndex].Values[0]);
            string description = (row.FindControl("txtdesc") as TextBox).Text;
            string postname = (row.FindControl("txtpostname") as TextBox).Text;
            DPCDetailsUpdate(dpcid,description,postname);
            grdDPCdetails.EditIndex = -1;
            BindGridviewDPCList();
        }
    }
